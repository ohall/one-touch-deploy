...
const createPath = function createStatusPath(error, params, endpoint) {
  return [
...
const doDeployment = function doDeployment (error, params, releaseID, callback) {
  ...
  let querystring = 'cms_deployment%5B' + releaseId + '%5D=214881978&' +
  ...
  let req = https.request(options, (response) => {
    ...
    response.on('data', (d) => { body += d; });
    response.on('end', () => {
      var deploymentId = JSON.parse(body).deploymentId;
